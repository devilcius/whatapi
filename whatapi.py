...
if sys.version_info.major < 3:
	raise "must use python 3 or greater"
import hashlib
try:
	from bs4 import BeautifulSoup
except:
	raise ImportError("Please install BeautifulSoup 4 module from https://www.crummy.com/software/BeautifulSoup/bs4/download/")
import http.client
import os
import pickle
import re
...
class Utils():
	...
	def md5(self, text):
		...
		h = hashlib.md5()
		h.update(self._string(text).encode('utf-8'))
...
class Request(object):
	...
	def downloadResponse(self):
		...
		conn = http.client.HTTPSConnection(self.whatcd.site, context=ssl._create_unverified_context())
		rb = ResponseBody()

		if self.whatcd.isProxyEnabled():
			conn = http.client.HTTPSConnection(host=self.whatcd.getProxy()[0], port=self.whatcd.getProxy()[1])
		...
		rb.body = re.sub('<script type="text/javascript">[^<]+</script>', '', response.read().decode('utf-8'))
		conn.close()
		return rb
...
class Authenticate(WhatBase):
	...
	def setCookie(self):
		...
		data = urllib.parse.urlencode(loginform)
	...
	def getAuthenticatedUserInfo(self, homepage=None):
		...
		"""
		if not homepage:
			homepage = BeautifulSoup(self._request("GET", "/index.php", "", self.whatcd.headers).execute(True).body, 'html.parser')
...
class User(WhatBase):
	...
	def getUserId(self):
		...
		if self.userid:
			...
			data = urllib.parse.urlencode(idform)
	...
	def getInfo(self):
		...
		if self.getUserId():
			...
			userpage = BeautifulSoup(self._request("GET", self.userpage + data, "", self.whatcd.headers).execute(True).body, 'html.parser')
	...
	def getTorrentsSeeding(self, page=1):
		...
		torrentspage = BeautifulSoup(self._request("GET", url, "", self.whatcd.headers).execute(True).body, 'html.parser')
...
class Torrent(WhatBase):
	...
	def getTorrentUrl(self):
		...
		if dict(headers) is None:
			...
			if 'Location' not in list(dict(headers).keys()):
				return None
			else:
	...
	def getInfo(self):
		...
		torrentpage = BeautifulSoup(self._request("GET", "/" + self.getTorrentUrl(), "", self.whatcd.headers).execute(True).body, 'html.parser')
...
class Artist(WhatBase):
	...
	def getInfo(self):
		...
		if self.getArtistId():
			...
			data = urllib.parse.urlencode(form)
			artistpage = BeautifulSoup(self._request("GET", self.artistpage + "?" + data, "", self.whatcd.headers).execute(True).body, 'html.parser')
	...
	def setArtistInfo(self, id, info):
		...
		if info[0]:
			...
			edit_page = BeautifulSoup(self._request("GET", self.artistpage + "?" + data, "", self.whatcd.headers).execute(True).body, 'html.parser')
			...
			response = self._request("POST", self.artistpage, urllib.parse.urlencode(data_to_post), self.whatcd.headers).execute(False)
...
class Parser(object):
	...
	def torrentInfo(self, dom, id, isparent):
		...
		if isparent:
			...
			torrentInfo['torrent']['rlstype'] = regrlstype.search(soup.find('div', {'class':'header'}).find('h2').contents[-1]).group(0)
...
#            torrentInfo['torrent']['commentspages'] = pages
	...
	def torrentsList(self, dom):
		...
		if torrentssoup:
			...
			for torrent in torrentssoup.find_all('tr')[1:]:
				...
				if 'cats_music' in torrent.find('td').find('div')['class']:
					...
					torrenttd = BeautifulSoup(torrenttd, 'html.parser')
					isScene = False
					info = ""
	...
	def postsList(self, dom):
		...
		if postssoup:
			...
			for post in postssoup.fetch('table', {'class':'forum_post box vertical_margin'}):
				...
				comment = ''.join([commentbody.string for commentbody in commentbody.findAll(text=True)])
